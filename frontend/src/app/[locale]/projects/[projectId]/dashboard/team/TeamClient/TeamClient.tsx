...
export default function TeamClient(): ReactElement {
  ...
  useEffect(() => {
    ...
    if (shouldRedirect) {
      router.replace(`/projects/${projectId}/dashboard/team?tab=${activeTab}`);
    }
  }, [isLoading, tab, activeTab, projectId, router]);
  ...
  const changeTab = (tb: TeamTabType): void => {
    ...
    router.replace(`/projects/${projectId}/dashboard/team?tab=${tb}`);
  };
  ...
  const handleDeleteMember = async (userId: string): Promise<void> => {
    ...
    try {
      await deleteMember({ id: projectId, userId }).unwrap();
  ...
  const deleteVacancy = async (roleId: string): Promise<void> => {
    ...
    try {
      await deleteSlot({ id: roleId, projectId }).unwrap();
  ...
  const addVacancy = async (roleId: string): Promise<void> => {
    ...
    try {
      await addSlot({ id: roleId, projectId }).unwrap();
  ...
  const addRole = async (roleId: string): Promise<void> => {
    ...
    try {
      await addProjectRole({ projectId, roleTypeId: roleId }).unwrap();
  ...
  const handleAcceptRequest = async (
    ...
    try {
      await acceptRequest({ id: requestId, userId, projectId });
  ...
  const handleCancelInvite = async (
    ...
    try {
      await cancelInvite({ id: inviteId, userId, projectId }).unwrap();
